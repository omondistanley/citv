...
11. [Planning Tabs](#planning-tabs)

---

## Planning Tabs

- [CPU Benchmark Baseline (No GPU)](docs/cpu_benchmark_baseline.md)
- [Pathing and Trajectory Chat Transcript (Hybrid)](docs/chat_transcript_paths_trajectories.md)
- [Path geometry, trajectories & context (Apr 2026)](docs/path_trajectory_geometry_context_2026-04.md) — walkable fusion, pair/graph gates, intra-mask interior paths, 3D polyline lift, metrics (`invalid_ratio`, `energy_total`), ranking parity.
- [Path, Trajectory, Affordance, And Action-Manifold Updates](docs/path_updates.md)
...
## Region-Aware Pipeline Upgrade (Apr 2026)

### Objectives 

- Implement and validate a region-aware CITV flow that propagates depth regions into downstream outputs.
- Ensure segmentation tracks produce masks (fixing `Track[grounded_sam2] skipped (no masks)` / `Track[amg] skipped (no masks)` issues).
- Thread region context through:
  - scene graph object fields
  - relations, layers, and mask hierarchy
  - per-track metadata and artifact bundles
  - caption/prompt exports
- Run tests and verify expected outputs.

### Implemented / Confirmed Changes

#### Region partitioning and exports

- Added region partitioning support (depth-driven clustering + connected components) and object-to-region assignment.
- Region artifacts are produced per track:
  - `*_regions.json`
  - `*_regions.png`
  - `*_regions_overlay.png`
- Region block is included in `*_scene.json` when regions are enabled.

#### Object-level region attributes

- Objects now carry region-aware fields used by downstream stages:
  - `region_index`
  - `region_id`
  - `coordinates_3d_region_relative`
  - `region_depth_percentile`
  - `depth_plausibility_score`
  - optional `label_warning`

#### Relations / layers / hierarchy integration

- Relations generation incorporates region context and tiers:
  - relation gating supports `"all"` and `"intra_region_only"`
  - `relation_tier` can be attached on relation records
  - optional coarse region-layer edges can be appended into `relations.json`
- Existing downstream maps remain integrated:
  - relations map (`*_relations_map.png`)
  - layers map (`*_layers.png`)
  - mask hierarchy map (`*_mask_hierarchy.png`)

#### Caption / prompt bundle parity
...
- Region control switches in `config.py` were aligned to active usage:
  ...
  - `regions_rampp_crops_enabled = True`
  - `region_relation_mode = "all"` (or `"intra_region_only"` for stricter pairs)
  - `append_region_layer_relations = True`
  - `regions_reject_implausible_labels = True`

### Root-Cause Analysis Outcomes (Debugging)

#### Why tracks were previously skipped (`no masks`)

- Primary cause identified: SAM2 predictor initialization failure due to missing/blocked dependencies (notably Hydra path and SAM2 import context).
- In GroundedSAM2 flow, when predictor is unavailable, boxes can be present while masks are empty; this yields skipped tracks.

#### Environment / setup failures encountered

- `python` binary not found on host shell (`python3` required).
- Hugging Face pre-download step failed on Florence-2 due to disk quota (`Errno 122`).
- PyTorch runtime import failure observed:
  - `Artifact of type=precompile already registered in mega-cache artifact factory`
  - tied to Torch/TorchVision runtime/import state inconsistency in that environment.
- Pytest initially failed due to no writable temp dir while disk was full.

### Setup and dependency handling outcomes

- SAM2 installed in editable mode with no-build-isolation path to avoid heavy redundant build/dependency churn.
- Added Hydra-related dependency coverage in requirements/setup flow notes.
- Verified SAM2 checkpoint availability and import-path behavior under `scene_understanding.py` bootstrap.

### Test Results Captured

- Targeted suite executed:
  - `test_regions.py`
  - `test_scene_graph_schema.py`
  - `test_pix2seq_fallback.py`
  - `test_depth.py`
- Final status after environment stabilization:
  - **8 passed, 1 skipped**
- Skip reason: optional golden fixture set absent for one schema check path.

### Additional Test/Scaffold Fixes Captured

- Added missing package output wrappers expected by tests:
  - `scene_understanding/output/__init__.py`
  - `scene_understanding/output/scene_graph_builder.py`
  - `scene_understanding/output/savers.py`
...
1. **Separate region graph artifacts**
   - add dedicated `*_region_relations.json`
   - add dedicated `*_region_relations_map.png`
   - keep mixed object-level `relations` outputs for backward compatibility

2. **More detailed mask hierarchy outputs**
   - add `*_mask_hierarchy_detailed.json`
   - add `*_mask_hierarchy_levels.json`
   - retain existing simpler hierarchy output

3. **Caption enhancement path**
   - feed region summaries + region-region edges + detailed hierarchy into prompts
   - enforce tiered caption structure:
     - global scene
     - per-region summaries
     - cross-region interactions
     - uncertainty/plausibility notes

### Expected Benefits Summarized

- Better relation precision via region-aware gating.
- Better caption coherence (region-first narrative instead of flat object list).
- Better explainability and debugging through explicit region-specific maps/artifacts.
- Better consistency between hierarchy, layers, and relation narratives.

### Current Practical Caveats

- Keep sufficient free disk space before model downloads / pytest runs.
- Ensure torch + torchvision are version-compatible in a single coherent Python environment.
- Prefer `python3` invocations in this host environment.
...
│           GDINO text label > Florence-2 <OD> > RAM++         │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         ▼
┌──────────────────────────────────────────────────────────────┐
...
```bash
python -m scene_understanding --input_dir images --output_dir output_scene
```
