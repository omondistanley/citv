...
class DepthCoordinator:
    ...
    def load_or_infer_depth(
        ...
        Checks for fp16 NPZ cache first (smaller, faster to load), then falls back
        to the float32 NPY. Always writes both formats on a fresh inference.
        """
        output_dir.mkdir(parents=True, exist_ok=True)
        depth_npy = output_dir / f"{image_stem}_depth_metric.npy"
        depth_npz = output_dir / f"{image_stem}_depth.npz"
        height, width = image_rgb.shape[:2]

        if reuse_existing:
            # Prefer the compact fp16 NPZ cache when available.
            if depth_npz.exists():
                try:
                    data = np.load(str(depth_npz))
                    arr = data["depth"] if "depth" in data.files else list(data.values())[0]
                    metric_depth = np.asarray(arr, dtype=np.float32)
                    if metric_depth.shape[:2] != (height, width):
                        metric_depth = cv2.resize(metric_depth, (width, height),
                                                  interpolation=cv2.INTER_NEAREST)
                    print(f"  Reusing cached depth (fp16 npz): {depth_npz.name}")
                    return metric_depth
                except Exception as exc:
                    print(f"  [depth] fp16 npz cache unreadable ({exc}); falling through.")
            if depth_npy.exists():
        ...
        try:
            np.savez_compressed(str(depth_npz), depth=metric_depth.astype(np.float16))
        except Exception as exc:
            print(f"  [depth] fp16 npz write failed: {exc}")
        return metric_depth
