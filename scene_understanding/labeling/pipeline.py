...
class LabelingPipeline:
    ...
    def predict_relation(
        self, subject_mask: np.ndarray, obj_mask: np.ndarray, image_bgr: np.ndarray
    ) -> str:
        """
        Predict spatial relation between subject and object.

        Uses Florence2's colored overlay technique (RED=subject, BLUE=object).

        Args:
            subject_mask: Binary mask for subject (HxW)
            obj_mask: Binary mask for object (HxW)
            image_bgr: Original image in BGR format

        Returns:
            Predicted spatial relation string
        """
        if not self.florence2.active:
            return "near"

        return self.florence2.predict_relation(subject_mask, obj_mask, image_bgr)
