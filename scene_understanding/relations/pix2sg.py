...
class Pix2SGWrapper:
    ...
    def __init__(
        ...
        # Hard cap on Florence-2 relation calls — each call takes 2-5 s on MPS/CPU.
        # Prevents silent multi-minute hangs on scenes with many overlapping objects.
        self._max_f2_pairs: int = 60
    ...
    def predict(
        ...
        if self._florence2 is not None and self._florence2.active and len(detections) >= 2:
            n_pairs = len(detections) * (len(detections) - 1)
            print(f"  [Pix2SG] Florence-2 relation enrichment: up to {n_pairs} pairs "
                  f"(max {self._max_f2_pairs} calls)...")
            florence_triplets = self._enrich_with_florence2(image, detections)
            triplets.extend(florence_triplets)
            print(f"  [Pix2SG] Florence-2 enrichment done: {len(florence_triplets)} extra triplets")
    ...
    def _enrich_with_florence2(
        ...
        n = len(detections)
        contact_graph = getattr(self, "_contact_graph", None)
        use_contact_graph = isinstance(contact_graph, set) and bool(contact_graph)
        f2_calls = 0

        for i in range(n):
            if f2_calls >= self._max_f2_pairs:
                break
            sub = detections[i]
            sub_mask = sub.get("_sam2_mask_array")
            if sub_mask is None:
            ...
            for j in range(n):
                ...
                if use_contact_graph:
                    if (min(i, j), max(i, j)) not in contact_graph:
                        continue
                obj = detections[j]
                obj_mask = obj.get("_sam2_mask_array")
                if obj_mask is None:
                ...
                if not use_contact_graph and len(sub_box) >= 4 and len(obj_box) >= 4:
                ...
                if union == 0:
                    continue
                if (inter / union) < self._relation_min_mask_overlap:
                    # Allow near-contact pairs even with zero pixel overlap.
                    # A 15×15 dilation captures objects within ~7 px of each other.
                    kernel = np.ones((15, 15), np.uint8)
                    sub_dilated = cv2.dilate(sub_m.astype(np.uint8), kernel, iterations=1).astype(bool)
                    if not int(np.logical_and(sub_dilated, obj_m).sum()) > 0:
                        continue
                ...
                f2_calls += 1
                if f2_calls % 10 == 1:
                    print(f"  [Pix2SG] Florence-2 pair {f2_calls}/{self._max_f2_pairs}: "
                          f"{sub_label} → {obj_label}")
