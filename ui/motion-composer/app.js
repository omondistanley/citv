...
function init() {
  ...
  [els.showObjects, els.showRegions, els.showCorridor, els.showOcclusion, els.showActor].forEach((el) => el.addEventListener("input", draw));
  els.clearGeometryBtn.addEventListener("click", () => { state.geometry = []; state.region = []; state.preview = null; refresh(); });
...
async function loadScene(event) {
  const file = event.target.files?.[0];
  if (!file) return;
  state.scene = JSON.parse(await file.text());
  renderObjectList();
  refresh();
}
...
function onPointerMove(event) {
  ...
  const pt = canvasPoint(event);
  const target = state.tool === "region" ? state.region : state.geometry;
...
function selectNearestObject(pt) {
  ...
  for (const obj of objects) {
    ...
    if (!best || d < best.d) best = { obj, d };
  }
  if (best && best.d < 60) {
    state.selectedRequired.has(best.obj.id) ? state.selectedRequired.delete(best.obj.id) : state.selectedRequired.add(best.obj.id);
    renderObjectList();
  }
}
...
function refresh() {
  draw();
  const contract = state.preview || buildGroundedContract();
  els.contractPreview.textContent = JSON.stringify(contract, null, 2);
}

function draw() {
  ctx.clearRect(0, 0, els.canvas.width, els.canvas.height);
  if (state.image) ctx.drawImage(state.image, 0, 0, els.canvas.width, els.canvas.height);
  else drawEmptyState();
...
function drawEmptyState() {
  ...
  ctx.fillText("Upload an image to begin", els.canvas.width / 2, els.canvas.height / 2);
}
...
function drawObjects() {
  for (const obj of getObjects()) {
    ...
    if (Array.isArray(box) && box.length >= 4) {
      ctx.strokeStyle = state.selectedRequired.has(obj.id) ? "#8ad7ff" : "rgba(255,255,255,0.42)";
      ctx.lineWidth = state.selectedRequired.has(obj.id) ? 3 : 1;
      ctx.strokeRect(box[0], box[1], box[2] - box[0], box[3] - box[1]);
    }
    const c = obj.mask_centroid_2d || obj.centroid_2d;
    if (Array.isArray(c)) {
      ctx.fillStyle = "rgba(10, 13, 18, 0.78)";
      ctx.fillRect(c[0] - 4, c[1] - 18, 120, 18);
      ctx.fillStyle = "#eef3fb";
      ctx.font = "12px system-ui";
      ctx.textAlign = "left";
      ctx.fillText(obj.canonical_name || obj.label || obj.id || "object", c[0], c[1] - 5);
    }
  }
}
...
function drawRegions() {
  ...
  regions.forEach((region, i) => {
    ...
    ctx.beginPath();
    ctx.arc(c[0], c[1], 22 + (i % 3) * 6, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(${90 + i * 37 % 120}, ${120 + i * 43 % 100}, 255, 0.11)`;
    ctx.fill();
    ctx.strokeStyle = "rgba(138,215,255,0.35)";
    ctx.stroke();
  });
}

function drawCorridor() {
  const pts = currentGeometryPoints();
  if (pts.length < 2) return;
  ctx.save();
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  ctx.lineWidth = Number(els.corridorRadius.value) * 2;
  ctx.strokeStyle = "rgba(138,215,255,0.12)";
...
function drawUserGeometry() {
  ...
  } else drawPolyline(pts, false);
  pts.forEach((p, i) => {
    ctx.beginPath(); ctx.arc(p[0], p[1], i === 0 || i === pts.length - 1 ? 7 : 3, 0, Math.PI * 2); ctx.fill();
  });
  ctx.restore();
}

function drawOcclusionPreview() {
  const contract = state.preview || buildGroundedContract();
  const pts = contract.grounded_geometry?.adapted_polyline_2d || [];
  const layers = contract.rendering?.render_layers || [];
  pts.forEach((p, i) => {
    if (layers[i] === "partially_occluded" || layers[i] === "behind_object") {
      ctx.beginPath(); ctx.arc(p[0], p[1], 9, 0, Math.PI * 2);
      ctx.fillStyle = layers[i] === "behind_object" ? "rgba(255,125,125,0.38)" : "rgba(255,211,125,0.32)";
      ctx.fill();
    }
  });
}
...
function drawActorPreview() {
  ...
  const t = Number(els.timeSlider.value) / 100;
  const p = pointAt(pts, t);
  const scaleHint = contract.rendering?.depth_scale_hint || [];
  const scale = scaleHint[Math.floor(t * (scaleHint.length - 1))] || 1;
  ctx.save();
  ctx.translate(p[0], p[1]);
  ctx.globalAlpha = 0.92;
  if (state.actorAssetUrl && state.actorImage?.complete) {
    const w = 68 * scale;
    ctx.drawImage(state.actorImage, -w / 2, -w / 2, w, w);
  } else {
    ctx.fillStyle = "rgba(215,184,255,0.86)";
    ctx.beginPath();
    ctx.ellipse(0, 0, 24 * scale, 10 * scale, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#0c0f14";
    ctx.font = `${Math.max(10, 12 * scale)}px system-ui`;
    ctx.textAlign = "center";
    ctx.fillText((els.actorText.value || "actor").slice(0, 12), 0, 4);
  }
  ctx.restore();
}
...
function renderObjectList() {
  ...
  if (!objects.length) {
    ...
    els.objectList.textContent = "No objects found in scene JSON.";
    return;
  }
  els.objectList.className = "object-list";
  els.objectList.innerHTML = "";
  objects.slice(0, 80).forEach((obj) => {
    ...
    [
      ...
      btn.className = set.has(obj.id) ? "active" : "";
      btn.addEventListener("click", () => { set.has(obj.id) ? set.delete(obj.id) : set.add(obj.id); renderObjectList(); refresh(); });
      row.appendChild(btn);
    });
...
function buildGroundedContract() {
  const geometryPoints = currentGeometryPoints();
  const mode = state.tool === "region" && state.region.length >= 3 ? "region" : geometryPoints.length === 1 ? "point" : geometryPoints.length === 2 ? "start_end" : "polyline";
  ...
  const contract = {
    ...
    actor: {
      actor_text: els.actorText.value.trim(),
      actor_source: state.actorAssetUrl ? "uploaded_asset" : "text",
      asset_ref: state.actorAssetUrl ? "browser_uploaded_actor_asset" : null,
      visual_style: state.actorAssetUrl ? "source_preserving" : "photorealistic",
    },
    action_text: els.actionText.value.trim(),
    user_geometry: {
      mode,
      points: geometryPoints.map((p) => [round(p[0]), round(p[1])]),
      source: mode === "point" ? "user_tap" : mode === "region" ? "user_region" : "user_drawn",
      corridor_radius_px: Number(els.corridorRadius.value),
    },
    manifold_type: manifold,
    duration_s: Number(els.duration.value),
    source: "user_authored",
    policy: {
      preserve_user_geometry: els.preserveGeometry.checked,
      allow_path_bending: els.allowBending.checked,
      max_path_deviation_px: Number(els.maxDeviation.value),
      required_object_ids: [...state.selectedRequired],
      avoid_object_ids: [...state.selectedAvoid],
      must_render_behind_object_ids: [...state.selectedBehind],
    },
  };
...
function adaptContractInBrowser(contract) {
  const pts = resample(contract.user_geometry.points, 48);
  const objects = getObjects();
  ...
  const visibility = renderLayers.map((l) => l === "behind_object" ? 0.18 : l === "partially_occluded" ? 0.56 : 1);
  const supportTrace = pts.map((p) => nearestRegionLabel(p));
  const depthTrace = pts.map((p) => estimateDepth(p));
  const warnings = [];
  ...
  return {
    ...
    contract,
    grounded_geometry: {
      manifold_type: contract.manifold_type,
      user_polyline_2d: contract.user_geometry.points,
      adapted_polyline_2d: pts,
      corridor_radius_px: contract.user_geometry.corridor_radius_px,
      path_preservation_policy: contract.policy,
    },
    traces: {
      depth_trace_m: depthTrace,
      support_trace: supportTrace,
      visibility_profile: visibility,
      occluder_ids: occluderIds,
      semantic_trace: supportTrace.map((s) => s === "unknown" ? "semantic_context_unknown" : `${s}:action_context`),
    },
    rendering: {
    ...
    report: {
      ...
      preserved: ["raw_user_geometry", "actor_text_or_asset", "action_text", "duration_s"],
      adapted: ["resampled path for preview", "estimated support trace", "estimated occlusion/render layers"],
      warnings,
      scores: {
        user_geometry_preservation: 1,
        min_visibility: round(Math.min(...visibility), 3),
        support_known_ratio: round(supportTrace.filter((s) => s !== "unknown").length / Math.max(1, supportTrace.length), 3),
      },
    },
    nearest_scene_entities: { objects: nearestObjects(pts[0] || [0, 0]).slice(0, 8) },
    alternatives: [],
  };
}
