...
│           GDINO text label > Florence-2 <MORE_DETAILED_CAPTION> │
│           > RAM++ tag-based label                            │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         ▼
...
Place image(s) in `images/` and inspect `output_scene/<stem>/scene_graph/staged/<stem>_scene.json`.

The modulated runner produces a per-image folder
(`output_scene/<stem>/scene_graph/staged/`) with the canonical scene JSON,
caption tiers, affordance bundles, path/trajectory/action artifacts, and a
self-audit (`<stem>_artifact_manifest.json`). See
`output_scene_missing_artifacts_review.md` for the full contract and
`docs/path_updates.md` for design intent.
...
| **Florence-2** | Semantic labelling (`<MORE_DETAILED_CAPTION>`) + relation captions (label-injected `<CAPTION>`) | [Xiao et al., 2023](https://arxiv.org/abs/2311.06242) |
| **RAM++** | Tag-based semantic labelling fallback | [Zhang et al., 2023](https://arxiv.org/abs/2310.15200) |
